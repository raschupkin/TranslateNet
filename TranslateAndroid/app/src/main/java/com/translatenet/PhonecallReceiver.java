    ...
    public void onCallStateChanged(Context context, int state, String number) {
        ...
        switch (state) {
            case TelephonyManager.CALL_STATE_RINGING:
                ...
                savedNumber = number;
                if (activity != null)
                    activity.onIncomingCallRinging(context, number, callStartTime);
                break;
...
//                    isIncoming = false;
                    callStartTime = new Date();
                    if (activity != null)
                        activity.onIncomingCallOffHook(context, savedNumber, callStartTime);
                }
                break;
            case TelephonyManager.CALL_STATE_IDLE:
                if (lastState == TelephonyManager.CALL_STATE_RINGING) {
                    ...
                    if (activity != null)
                        activity.onIncomingCallEnded(context, savedNumber, callStartTime, new Date());
                } else {
                    if (activity != null)
                        activity.onOutgoingCallEnded(context, savedNumber, callStartTime, new Date());
                }
                break;
        }
        lastState = state;
