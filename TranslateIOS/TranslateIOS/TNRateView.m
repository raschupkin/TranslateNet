...
- (void)refresh {
    for(float i = 0; i < self.imageViews.count; ++i) {
        UIImageView *imageView = [self.imageViews objectAtIndex:i];
        if (self.rating >= i+0.75) {
...
-(float)normalizeRating:(float)rating
{
    ...
    if (rating < fr+0.25)
        return fr;
    else if (rating > fr+0.25 && rating < fr+0.75)
        return fr+0.5;
    else
        return fr+1;
}
...
- (void)handleTouchAtLocation:(CGPoint)touchLocation {
    ...
    for(int i = self.imageViews.count - 1; i >= 0; i--) {
        ...
        if (touchLocation.x > imageView.frame.origin.x) {
            newRating = i;
            newRating += (touchLocation.x - imageView.frame.origin.x) /
                            imageView.frame.size.width;
            break;
        }
    }
    
    self.rating = [self normalizeRating:newRating];
}
